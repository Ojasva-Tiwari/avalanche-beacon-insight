...
import { REPLAY_EVENTS } from "@/lib/mock/dataset";
import {
  incidentQuery,
  sensorsQuery,
  zoneDetailsQuery,
...
function ReplayPage() {
  ...
  const current = REPLAY_EVENTS[Math.min(step, REPLAY_EVENTS.length - 1)]!;
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-2">
            ...
            <PanelHeader
              ...
              right={
                <Tag className="border-border bg-muted/50 text-muted-foreground">
                  STEP {step + 1}/{REPLAY_EVENTS.length}
                </Tag>
              }
            />
            <PanelBody className="py-2">
              <div className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  onClick={() => {
                    setPlaying(false);
                    setStep((s) => Math.min(REPLAY_EVENTS.length - 1, s + 1));
                  }}
                  className="rounded-sm border border-border p-1.5 text-muted-foreground hover:text-foreground"
                >
                  <SkipForward className="size-3.5" aria-hidden />
                </button>
                <span className="num ml-2 text-[11px] text-foreground">T+{current.t}</span>
              ...
              <div className="mt-2">
                ...
                <p className="mt-1 text-[11px] leading-relaxed text-muted-foreground">{current.description}</p>
              </div>
            </PanelBody>
          </Panel>
          ...
          {incident && zones ? (
            <SearchMap
              ...
              scenario={scenario}
              contextEstablished={current.contextEstablished}
            />
          ) : (
